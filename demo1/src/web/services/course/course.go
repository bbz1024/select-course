...
func SelectCourse(ctx *gin.Context) {
	...
	if err := database.Client.WithContext(context.Background()).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("id=?", req.UserID).First(&user).Error; err != nil {
...
func BackCourse(ctx *gin.Context) {
	...
	if err := database.Client.WithContext(context.Background()).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("user_id=? and course_id=?", req.UserID, req.CourseID).First(&userCourse).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
		...
		var course models.Course
		if err := tx.Model(models.Course{}).Preload("Schedule").
			Where("id=?", req.CourseID).First(&course).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
		...
		if err := database.Client.Clauses(clause.Locking{Strength: "UPDATE"}).Where("id=?", req.UserID).First(&user).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				logger.Logger.WithContext(ctx).Info("用户不存在", err)
				resp.Fail(ctx, code.NotFound, code.UserNotFound, code.UserNotFoundMsg)
				return err
		...
		// 2.4 删除选课记录
		if err := tx.Where("user_id=? and course_id=?", req.UserID, req.CourseID).Delete(&models.UserCourse{}).Error; err != nil {

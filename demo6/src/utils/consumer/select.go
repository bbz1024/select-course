...
import (
	...
	"gorm.io/gorm"
	"gorm.io/gorm/clause"
	"select-course/demo6/src/constant/variable"
	"select-course/demo6/src/models"
	"select-course/demo6/src/models/mqm"
	"select-course/demo6/src/storage/cache"
	"select-course/demo6/src/storage/database"
	"select-course/demo6/src/storage/mq"
	"select-course/demo6/src/utils/local"
		...
		if !msg.Ack {
			...
			if err := retry.Do(func() error {
				if err := s.PushDeadQueue(data.Body); err != nil {
					cnt++
					logger.Logger.Error("消息发送失败%d次")
					return err
				}
				return nil
			}, retry.Attempts(3), retry.Delay(time.Millisecond*100)); err != nil {
				logger.Logger.Error("消息重新发送失败")
				// 丢入到死信队列，进行补偿操作
				if err := s.PushDeadQueue(data.Body); err != nil {
					logger.Logger.Error("消息发送失败")
				}
			}
		}
...
func updateUserCourseState(tx *gorm.DB, msg *mqm.CourseReq, selectAction bool) error {
	...
	if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).
		...
		if !errors.Is(err, gorm.ErrRecordNotFound) {
			logger.Logger.Error("获取用户选课记录失败")
			return err
		}
		// 不存在，则创建
